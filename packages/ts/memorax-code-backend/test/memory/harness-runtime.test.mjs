...
import { createPendingQuotaNoticeRuntime } from "../../dist/memory/quota-notice.js";

test("turn start never searches with the legacy flag and still delivers pending Add notices", async () => {
  const root = await mkdtemp(join(tmpdir(), "memorax-harness-turn-start-"));
  const requests = [];
  const claims = [];
  const pendingQuotaNotice = createPendingQuotaNoticeRuntime({
    claimQuotaNotice: async (_config, quota) => {
      ...
      return "Pending Add quota notice.";
    },
  });
  const runtime = createHarnessMemoryRuntime(definition("claude-code"), {
    ...
    pendingQuotaNotice,
    fetchImpl: async (url) => { requests.push(url); throw new Error("turn start must not search"); },
  });
  const turn = { sessionId: "session", clientTurnId: "turn", cwd: root, createdAt: Date.now(), prompt: "Recall the parser boundary." };
  try {
    const repositoryMemory = await runtime.resolveRepositoryMemory(turn);
    assert.equal(repositoryMemory.ok, true);
    pendingQuotaNotice.queue(repositoryMemory.memory.config, { featureCode: "memory_write", remaining: 10, limit: 100 });
    assert.deepEqual(await runtime.recordTurnStart({ ...turn, clientTurnId: undefined }), { ok: true });
    assert.equal(runtime.size(), 0);
    assert.deepEqual(claims, []);
    assert.deepEqual(requests, []);

    assert.deepEqual(await runtime.recordTurnStart(turn), { ok: true, userNotice: "Pending Add quota notice." });
    assert.equal(runtime.size(), 1);
    assert.deepEqual(await runtime.recordTurnStart(turn), { ok: true });
    assert.deepEqual(claims, ["memory_write"]);
    assert.deepEqual(requests, []);
  } finally {
    runtime.close();
...
test("harness runtimes isolate identical turns and preserve injected resources when one closes", async () => {
  ...
  const options = {
    ...
    fetchImpl: async () => { throw new Error("turn start must not search"); },
  };

...
test("config seeding appends disabled Jev to legacy configs once without replacing their settings", async (t) => {
  for (const newline of ["\n", "\r\n"]) {
    await t.test(newline === "\n" ? "LF" : "CRLF", async (t) => {
      const root = await mkdtemp(join(tmpdir(), "memorax-code-config-jev-upgrade-"));
      t.after(() => rm(root, { recursive: true, force: true }));
      const configPath = join(root, "config.toml");
      const original = [
        "# Keep existing credentials and comments.",
        "[memorax]",
        'api_key = "fixture-existing-memory-key"',
        'user_id = "fixture-existing-user"',
        "[memory.writeback]",
        "enabled = false",
        "[unrelated]",
        'jev = "not a root-level Jev setting"',
      ].join(newline);
      await writeFile(configPath, original, { mode: 0o640 });
      const before = await stat(configPath);

      assert.equal(await seedMissingMemoraxCodeConfig(root), false, "upgrading an existing file is not initial creation");
      const upgraded = await readFile(configPath, "utf8");
      assert.equal(upgraded.startsWith(original + newline + newline), true);
      assert.match(upgraded, /\[jev\]\r?\nenabled = false/);
      assert.deepEqual(loadMemoraxCodeConfig(root).jev, { enabled: false });
      assert.equal(loadMemoraxCodeConfig(root).memorax.api_key, "fixture-existing-memory-key");
      assert.equal(loadMemoraxCodeConfig(root).memory.writeback.enabled, false);
      if (newline === "\r\n") assert.equal(upgraded.replaceAll("\r\n", "").includes("\n"), false);
      if (process.platform !== "win32") {
      ...
      assert.equal(await seedMissingMemoraxCodeConfig(root), false);
      assert.equal(await readFile(configPath, "utf8"), upgraded);
    });
  }
});

test("config seeding preserves every existing root Jev form byte-for-byte", async (t) => {
  const cases = [
    ["configured table", '[jev]\nenabled = true # Keep this enabled.\napi_key = "fixture-existing-jev-key"\n'],
    ["partial table", "[jev]\nenabled = true\n"],
    ["empty table", "[jev]\n"],
    ["inline table", 'jev = { api_key = "fixture-existing-jev-key" }\n'],
    ["dotted fields", 'jev.enabled = true\njev.api_key = "fixture-existing-jev-key"\n'],
    ["quoted key", '"jev".enabled = true\n'],
    ["scalar value", "jev = false\n"],
    ["array value", "jev = []\n"],
  ];
  for (const [name, original] of cases) {
    await t.test(name, async (t) => {
      const root = await mkdtemp(join(tmpdir(), "memorax-code-config-jev-preserve-"));
      t.after(() => rm(root, { recursive: true, force: true }));
      const configPath = join(root, "config.toml");
...
test("config seeding rejects malformed TOML without changing or exposing its content", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "memorax-code-config-jev-invalid-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const configPath = join(root, "config.toml");
  ...
  await assert.rejects(seedMissingMemoraxCodeConfig(root), {
    message: "failed to safely update MemoraX Code config",
  });
  assert.equal(await readFile(configPath, "utf8"), original);
  assert.doesNotMatch(warnings.join("\n"), /fixture-private-key-must-not-appear|api_key/);
});

test("config seeding preserves an unwritable legacy config and reports a safe failure", {
  skip: process.platform === "win32" || process.getuid?.() === 0,
}, async (t) => {
  const root = await mkdtemp(join(tmpdir(), "memorax-code-config-jev-readonly-"));
  const configPath = join(root, "config.toml");
  const original = '[memorax]\napi_key = "fixture-preserved-memory-key"\n';
  await writeFile(configPath, original);
